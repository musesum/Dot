 ...
 *   p'(0) = t0, p'(1) = t1.
 */
void CubicPoly::InitCubicPoly(float x0, float x1, float t0, float t1, CubicPoly &p)
{
    p.c0 = x0;
    p.c1 = t0;
...
void CubicPoly::InitCatmullRom(float x0, float x1, float x2, float x3, CubicPoly &p)
{
	// Catmull-Rom with tension 0.5
    CubicPoly::InitCubicPoly(x1, x2, 0.5f*(x2-x0), 0.5f*(x3-x1), p);
}

// compute coefficients for a nonuniform Catmull-Rom spline
void CubicPoly::InitNonuniformCatmullRom(float x0, float x1, float x2, float x3, float dt0, float dt1, float dt2, CubicPoly &p)
{
...
float CubicPoly::VecDistSquared(const Vec2D& p, const Vec2D& q)
{
	float dx = q.x - p.x;
	float dy = q.y - p.y;
	return dx*dx + dy*dy;
...
void CubicPoly::InitCentripetalCR(const Vec2D& p0, const Vec2D& p1, const Vec2D& p2, const Vec2D& p3,
